...
int main(void){
	...
	char *login, *mdp, *query, *message_erreur, *totpsecret, *activite, *code, *totpcode, *salon, *gerant, *nom ;
	int succes = 0; //succes=0 --> échec // succes=1 --> Verifications OK
	int idmaxi; //Pour stocker un nombre de réponses
	...
	nom            = (char*)malloc(query_size+20000); //Nom de l'utilisateur à ajouter

	get_chaine(envoi, 1, nom);
	get_chaine(envoi, 2, gerant);
	get_chaine(envoi, 3, salon);
	...
	if (succes == 1){
		...
		sprintf(query, "INSERT INTO Abonne VALUES ('%s', '%s', '%s', 1, 0);", nom, salon, gerant);
		db_query(handler, query);
		...
		db_clear_result(result);
		sprintf(query, "INSERT INTO Message VALUES ('Admin', '%s', '%s', '%s a rejoint le salon', '', '', '', '', '', '%s', %d);", salon, gerant, nom, horodatage, idmaxi+1);
		db_query(handler, query);

		printf("<body><div data-role=\"page\" data-theme=\"b\"><h5>Vous avez bien invité %s dans le salon.</h5><br>\n\
			...
			</div></div>\n", nom, login, mdp, code, salon, gerant);
	}
	printf("</div></body>\n</html>\n");
