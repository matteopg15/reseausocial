...
int main(void){
	...
	nom            = (char*)malloc(query_size+20000); //Nom de l'utilisateur à activer
	...
	if (succes==1){
		...
		db_clear_result(result);
		sprintf(query, "UPDATE Compte SET actif=true WHERE nom='%s';",nom);
		db_query(handler,query);
		printf("<body data-theme='b'><div data-role=\"page\" data-theme=\"b\">\n\
			<h5>Le compte de <italic>%s</italic> a bien été activé !</h5>\n\
