...
int main(void){
	...
	char *login, *mdp, *query, *message_erreur, *totpsecret, *activite, *code, *totpcode, *salon, *gerant, *buf ;
	int succes = 0; //succes=0 --> échec // succes=1 --> Verifications OK
	int idmess; // Identifiant du message
	...
	salon          = (char*)malloc(query_size+20000); //Salon actuel
	gerant         = (char*)malloc(query_size+20000); //gerant du salon
	buf            = (char*)malloc(query_size+20000); //gerant du salon


	
	get_chaine(envoi, 1, buf);
	idmess = atoi(buf);
	...
	db_clear_result(result);
	sprintf(query, "DELETE FROM Message WHERE salon='%s' AND gerant='%s' AND id=%d;", salon, gerant, idmess);
	db_query(handler, query);

	db_clear_result(result);
	sprintf(query, "SELECT * FROM Message WHERE salon='%s' AND gerant='%s';", salon, gerant);
	db_query(handler, query);
	int nb_messages = db_ntuples(result);
	int idmaxi      = 1;
	...
	if (nb_messages>0){
		...
		db_getvalue(result, 0, 0, buf, 2000);
		idmaxi = atoi(buf);
	}else idmaxi = 1;
	
	db_clear_result(result);
	sprintf(query, "UPDATE Abonne SET idlast=%d WHERE salon='%s' AND gerant='%s' AND idlast>%d;", idmaxi, salon, gerant, idmaxi);
	db_query(handler, query);

	db_clear_result(result);
	sprintf(query, "UPDATE Abonne SET idfirst=%d WHERE salon='%s' AND gerant='%s' AND idfirst>%d;", idmaxi, salon, gerant, idmaxi);
	db_query(handler, query);
