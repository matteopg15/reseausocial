...
int main(void){
	...
	int idmaxi, idminabo; //ID du message le plus recent // ID du message non vu le plus vieux
	envoi = read_POST();
	...
	if (succes == 1){
		...
		//Marquer tout lu
		db_clear_result(result);
		sprintf(query, "UPDATE Abonne SET idlast=%d, idfirst=%d WHERE compte='%s' AND salon='%s' AND gerant='%s' ;", idmaxi, idmaxi+1, login, salon, gerant);
		db_query(handler, query);

		//Auto-destruction
		db_clear_result(result);
		sprintf(query, "SELECT MIN(idfirst) FROM Abonne WHERE salon='%s' AND gerant='%s';", salon, gerant);
		db_query(handler, query);
		db_getvalue(result, 0, 0, buf, 20000);
		idminabo  = atoi(buf);

		db_clear_result(result);
		sprintf(query, "DELETE FROM Message WHERE salon='%s' AND gerant='%s' AND id<%d;", salon, gerant, idminabo);
		db_query(handler, query);
