...
int main(void){
	...
	char *login, *mdp, *query, *message_erreur, *totpsecret, *activite, *code, *totpcode, *nvmdp, *nvmdp2, *mdpactuel;
	...
	nvmdp          = (char*)malloc(query_size+20000); //Nouveau mdp
	nvmdp2         = (char*)malloc(query_size+20000); //Confirmation nouveau mdp
	mdpactuel      = (char*)malloc(query_size+20000); //Mot de passe actuel saisi par l'util

	get_chaine(envoi, 1, nvmdp2);
	get_chaine(envoi, 2, nvmdp);
	get_chaine(envoi, 3, mdpactuel);
	get_chaine(envoi, 4, code);
	get_chaine(envoi, 5, mdp);
	get_chaine(envoi, 6, login);
		...
		if (db_ntuples(result)!= 1) strcpy(message_erreur,"Cet utilisateur n'existe pas.");        //Test utilisateur existe
			...
			if (check_password(mdp, mdp_bdd)==1) strcpy(message_erreur,"Mauvais mot de passe.");   //Vérification mot de passe
				...
				if (strcmp(activite, "0")==0) strcpy(message_erreur, "Votre compte n'a pas été activé par l'administrateur.") ;
					...
					if(strcmp(mdpactuel, mdp)!=0) strcpy(message_erreur, "Le mot de passe actuel saisi n'est pas bon.");
					else{
						if(strcmp(nvmdp, nvmdp2)!=0) strcpy(message_erreur, "Les deux mots de passe ne correspondent pas.");
						else{
							db_clear_result(result);
	...
	if (succes == 0){
		...
		db_clear_result(result);
		hash256(nvmdp);
		sprintf(query, "UPDATE Compte SET mdp='%s' WHERE nom='%s';", nvmdp, login);
		db_query(handler, query);

		printf("<body data-theme='b'><div data-role=\"page\" data-theme=\"b\">\n\
			<h5>Changement effectué !</h5>\n\
			<a href=\"/reseausocial/index.html\"><button data-mini=\"false\" data-inline=\"false\" class=\"ui-shadow ui-btn ui-btn-b ui-btn-icon-top\">Changer le mot de passe</button></a></div>\n");
	}
}