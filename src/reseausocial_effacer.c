...
int main(void){
	...
	if (succes == 0){
		...
		db_clear_result(result);
		sprintf(query, "DELETE FROM Message;");
		db_query(handler, query);
		db_clear_result(result);
		sprintf(query, "DELETE FROM Abonne;");
		db_query(handler, query);
		db_clear_result(result);
		sprintf(query, "DELETE FROM Salon;");
		db_query(handler, query);
		db_clear_result(result);
		sprintf(query, "DELETE FROM Compte;");
		db_query(handler, query);

		printf("<body data-theme='b'>\n\
		<div data-role=\"page\" data-theme=\"b\">\n\
		La base de données a bien été effacée\n\
		<a href=\"/reseausocial/index.html\"><button type=\"submit\" data-mini=\"false\" data-inline=\"false\" data-icon=\"back\" class=\"ui-shadow ui-btn ui-btn-b ui-icon-back ui-btn-icon-top\"></button></a>\n\
		</div>\n\
		");
