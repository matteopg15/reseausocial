...
int main(void){
	...
	char *login, *mdp, *query, *message_erreur, *totpsecret, *activite, *code, *totpcode, *nom;
	...
	nom            = (char*)malloc(query_size+20000); //Nom du salon à créer
	...
	if (succes ==1){
		...
		db_clear_result(result);
		sprintf(query, "SELECT nom FROM Salon WHERE nom='%s' AND gerant='%s';", nom, login);
		db_query(handler, query);
		if(db_ntuples(result)!= 0) strcpy(message_erreur, "Vous avez déjà un salon nommé ainsi.");
		else{

			//Création salon
			db_clear_result(result);
			sprintf(query, "INSERT INTO Salon VALUES ('%s', '%s');", nom, login);
			db_query(handler, query);

			//Le gérant est membre du salon
			db_clear_result(result);
			sprintf(query, "INSERT INTO Abonne VALUES ('%s', '%s', '%s', 1, 0);", login, nom, login);
			db_query(handler, query);

			//Admin est membre de tous les salons
			db_clear_result(result);
			sprintf(query, "INSERT INTO Abonne VALUES ('Admin', '%s', '%s', 1, 0);", nom, login);
			db_query(handler, query);
			...
			sprintf(query, "SELECT nom FROM Salon WHERE nom='%s' AND gerant='%s';", nom, login);
			db_query(handler, query);
			if(db_ntuples(result) == 0 ) strcpy(message_erreur, "Le salon n'a pas pu être créé, quelque chose s'est mal passé.");
			else succes = 2;
		}
	}
		...
		printf("<body data-theme='b'>\n\
			...
			<div data-role='fieldcontain'>\n\
				Salon créé avec succès !\n\
					...
					<input id='text-code' name='text-entree' type='hidden' value='n'>\n\
					<br><br><br><br>\n\
					<button type=\"submit\" data-mini='false' data-inline='true' data-icon=\"arrow-r\">Continuer</button>\n\
				</div>\n\
			</form></div>\n", login, mdp, code);
	}
