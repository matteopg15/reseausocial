...
int main(void){
	...
	nom            = (char*)malloc(query_size+20000); 
	...
	if (succes == 1){
		...
		db_clear_result(result);
		sprintf(query, "DELETE FROM Abonne WHERE compte='%s' AND salon='%s' AND gerant='%s';", nom, salon, gerant);
		db_query(handler, query);
		...
		db_clear_result(result);
		sprintf(query, "INSERT INTO Message VALUES ('Admin', '%s', '%s', '%s a été exclu du salon', '', '', '', '', '', '%s', %d);", salon, gerant, nom, horodatage, idmaxi+1);
		db_query(handler, query);

		printf("<body><div data-role=\"page\" data-theme=\"b\"><h5>Vous avez bien exclu %s du salon.</h5><br>\n\
