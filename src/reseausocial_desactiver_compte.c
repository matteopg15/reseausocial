...
int main(void){
	...
	nom            = (char*)malloc(query_size+20000); //Nom de l'utilisateur à désactiver
	...
	if (succes==1){
		...
		db_clear_result(result);
		sprintf(query, "UPDATE Compte SET actif=false WHERE nom='%s';",nom);
		db_query(handler,query);
		printf("<body data-theme='b'><div data-role=\"page\" data-theme=\"b\">\n\
			<h5>Le compte de <italic>%s</italic> a bien été désactivé !</h5>\n\
			...
			<input type=\"hidden\" name=\"txt-code\" id=\"txt-code\" value=\"n\">\n\
			<button type='submit' data-mini='false' data-iniline='false' data-theme='b' data-icon=\"back\" class='ui-shadow'>Retourn à la liste des salons</button>\n\
			</form></div>\n\
			\n", nom, login, mdp, code);
