...
int main(void){
	...
	if (succes == 0){
		...
		db_query(handler, query);
		db_clear_result(result);
		sprintf(query, "UPDATE Compte SET avatar='%s' WHERE nom='%s';", url_avatar, login);
		db_query(handler, query);
		...
		printf("<body data-theme='b'><div data-role=\"page\" data-theme=\"b\">\n\
			...
			<button type=\"submit\" data-mini='false' data-inline='false' data-icon=\"lock\">Retour au profil</button>\n\
