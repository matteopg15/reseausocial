...
int main(void){
	...
	if (succes == 1){
		...
		int nb_comptes, nb_actifs, nb_messages, nb_salons;

		db_clear_result(result);
		sprintf(query, "SELECT * FROM Compte;");
		db_query(handler, query);
		nb_comptes = db_ntuples(result);

		db_clear_result(result);
		sprintf(query, "SELECT * FROM Compte WHERE actif=true;");
		db_query(handler, query);
		nb_actifs = db_ntuples(result);

		db_clear_result(result);
		sprintf(query, "SELECT * FROM Message;");
		db_query(handler, query);
		nb_messages = db_ntuples(result);

		db_clear_result(result);
		sprintf(query, "SELECT * FROM Salon;");
		db_query(handler, query);
		nb_salons = db_ntuples(result);

		printf("Sur l'application, il y a :<br>\n\
			<ul>\n\
				<li>Il y a actuellement <b>%d utilisateurs</b> inscrits sur la plate-forme dont <b>%d actifs</b></li><br>\n\
				<li><b>%d messages</b> ont été envoyés sur les <b>%d salons</b> que comporte la plate-forme</li>\n\
			</ul>", nb_comptes, nb_actifs, nb_messages, nb_salons);
