...
int main(void){
	...
	int nb; //Pour stocker un nombre de réponses
		...
		if (db_ntuples(result)!= 1) strcpy(message_erreur,"Cet utilisateur n'existe pas.");        //Test utilisateur existe
			...
			if (check_password(mdp, mdp_bdd)==1) strcpy(message_erreur,"Mauvais mot de passe.");   //Vérification mot de passe
				...
				if(db_ntuples(result)==0) strcpy(message_erreur, "Vous n'êtes pas membre de ce salon.");
					...
					if (strcmp(login, gerant)!=0) strcpy(message_erreur, "Ce menu est réservé au gérant du salon");
					else{
						db_clear_result(result);
	...
	if (succes == 1){
		...
		//Liste des membres (qui peuvent être éjectés)
		printf("<body><div data-role=\"page\" data-theme=\"b\">\n\
			<h4>Membres du salons : </h4>\n\
		<form autocomplete=false action='/cgi-bin/reseausocial_supprimer_membre.cgi' method='POST'>\n\
		...
		<div class=\"ui-controlgroup-controls \">", login, mdp, code, salon, gerant);

		db_clear_result(result);
		sprintf(query, "SELECT compte FROM Abonne WHERE salon='%s' AND gerant='%s';", salon, gerant);
		db_query(handler,query);
		...
		for (int i=0; i<nb; i++){
			...
			if (strcmp(nom, gerant)!=0 && strcmp(nom, "Admin")!=0){
				printf("<div class=\"ui-radio\">\n\
							<label for=\"radio-choice-supprimer-%d\" class=\"ui-btn ui-corner-all ui-btn-inherit ui-btn-icon-left ui-radio-on ui-first-child\">%s</label>\n\
							<input type=\"radio\" name=\"radio-choice-supprimer\" id=\"radio-choice-supprimer-%d\" value=\"%s\" >\n\
						</div>\n", i, nom, i, nom);
			}else;

		}
		printf("</div></fieldset><br><button type='submit' data-mini='false' data-iniline='true' data-theme='b' data-icon=\"delete\" class='ui-shadow'>Exclure du salon</button></form>");

		//Liste des non membres (qui peuvent rejoindre)
		//SELECT t1.nom FROM Compte AS t1 LEFT JOIN Abonne AS t2 ON t2.compte=t1.nom AND t2.salon='xyz' AND t2.gerant='matteo' WHERE t2.compte is NULL AND t1.actif=true;
		printf("<h4>Utilisateurs non membres du salon : </h4>\n\
		<form autocomplete=false action='/cgi-bin/reseausocial_ajouter_membre.cgi' method='POST'>\n\
		<input type=\"hidden\" name=\"txt-login\" id=\"txt-login\" value=\"%s\">\n\
		<input type=\"hidden\" name=\"txt-mdp\" id=\"txt-mdp\" value=\"%s\">\n\
		<input type=\"hidden\" name=\"txt-code\" id=\"txt-code\" value=\"%s\">\n\
		<input type=\"hidden\" name=\"txt-salon\" id=\"txt-salon\" value=\"%s\">\n\
		...
		db_clear_result(result);
		sprintf(query, "SELECT t1.nom FROM Compte AS t1 LEFT JOIN Abonne AS t2 ON t2.compte=t1.nom AND t2.salon='%s' AND t2.gerant='%s' WHERE t2.compte is NULL AND t1.actif=true;", salon, gerant);
		db_query(handler,query);
		...
		for (int i=0; i<nb; i++){
			...
			db_getvalue(result, i, 0, nom, 20000);
				...
				printf("<div class=\"ui-radio\">\n\
							<label for=\"radio-choice-ajouter-%d\" class=\"ui-btn ui-corner-all ui-btn-inherit ui-btn-icon-left ui-radio-on ui-first-child\">%s</label>\n\
							<input type=\"radio\" name=\"radio-choice-ajouter\" id=\"radio-choice-ajouter-%d\" value=\"%s\" >\n\
						</div>\n", i, nom, i, nom);

		}
		printf("</div></fieldset><br><button type='submit' data-mini='false' data-iniline='true' data-theme='b' data-icon=\"check\" class='ui-shadow'>Inviter dans le salon</button></form>");

		//Supprimer salon
		printf("<br><br>\n\
				<form autocomplete=false action='/cgi-bin/reseausocial_supprimer_salon.cgi' method='POST'>\n\
				<input type=\"hidden\" name=\"txt-login\" id=\"txt-login\" value=\"%s\">\n\
				<input type=\"hidden\" name=\"txt-mdp\" id=\"txt-mdp\" value=\"%s\">\n\
				<input type=\"hidden\" name=\"txt-code\" id=\"txt-code\" value=\"%s\">\n\
				<input type=\"hidden\" name=\"txt-salon\" id=\"txt-salon\" value=\"%s\">\n\
				<input type=\"hidden\" name=\"txt-gerant\" id=\"txt-gerant\" value=\"%s\">\n\
				<button type='submit' data-mini='false' data-iniline='true' data-theme='b' data-icon=\"alert\" class='ui-shadow'>Supprimer le salon</button>\n\
		...
		printf("<div data-role=\"footer\" data-position=\"fixed\" data-theme=\"b\" role=\"contentinfo\" class=\"ui-footer ui-bar-b ui-footer-fixed slideup\">\n\
			...
			<ul class=\"ui-grid-c\">\n\
               <li class=\"ui-block-a\">\n\
                  	...
                  	<button type='submit' data-mini='false' data-iniline='true' data-theme='b' data-icon=\"back\" class='ui-shadow'></button>\n\
				  	<input type=\"hidden\" name=\"txt-login\" id=\"txt-login\" value=\"%s\">\n\
					<input type=\"hidden\" name=\"txt-mdp\" id=\"txt-mdp\" value=\"%s\">\n\
					<input type=\"hidden\" name=\"txt-code\" id=\"txt-code\" value=\"%s\">\n\
					<input type=\"hidden\" name=\"txt-salon\" id=\"txt-salon\" value=\"%s\">\n\
